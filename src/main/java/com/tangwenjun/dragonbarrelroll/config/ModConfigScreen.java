...
public class ModConfigScreen extends Screen {
    ...
    protected void init() {
        ...
        doneButton = Button.builder(
                ...
                .size(80, 20).build();
        // NOT added as renderable widget — rendered manually after scissor disable
    ...
    public void extractRenderState(@NotNull GuiGraphicsExtractor graphics, int mouseX, int mouseY, float partialTick) {
        ...
        // Render scrollable widgets manually with per-widget visibility check
        // (same approach as old working version — avoids clipping the title)
        ...
        for (var sw : scrollWidgets) {
            ...
            if (wy + wh > CLIP_TOP && wy < clipBottom) {
                sw.widget.extractRenderState(graphics, mouseX, mouseY, partialTick);
            }
        }
        graphics.disableScissor();

        // Re-render pinned buttons outside scissor
        doneButton.extractRenderState(graphics, mouseX, mouseY, partialTick);
        resetAllButton.extractRenderState(graphics, mouseX, mouseY, partialTick);

        // Tooltips (using 26.1.2 setTooltipForNextFrame API)
        // Only show tooltips for widgets within the visible clip area
        if (mouseY >= CLIP_TOP && mouseY <= clipBottom) {
            for (var sw : scrollWidgets) {
                if (sw.tooltipKey != null && sw.widget.isMouseOver(mouseX, mouseY)) {
                    graphics.setTooltipForNextFrame(this.font,
                            this.font.split(Component.translatable(sw.tooltipKey), 200),
                            mouseX, mouseY);
                    break;
                }
            }
    ...
    public boolean mouseClicked(MouseButtonEvent event, boolean isFocused) {
        ...
        if (mouseY >= CLIP_TOP && mouseY <= clipBottom) {
            for (var sw : scrollWidgets) {
                if (sw.widget.isMouseOver(mouseX, mouseY)) {
                    if (sw.widget.mouseClicked(event, isFocused)) {
        ...
        // Pinned buttons (outside scissor, not in widget tree)
        if (doneButton.isMouseOver(mouseX, mouseY) && doneButton.mouseClicked(event, isFocused)) {
            return true;
        }
        if (resetAllButton.isMouseOver(mouseX, mouseY) && resetAllButton.mouseClicked(event, isFocused)) {
            return true;
        }

        // Only let super handle clicks within the clip area — prevents
        // scrolled-out widgets from being clickable through the Screen base class
        if (mouseY >= CLIP_TOP && mouseY <= clipBottom) {
    ...
    public boolean mouseReleased(MouseButtonEvent event) {
        ...
        // Forward to scrollable widgets (same pattern as old working version)
        for (var sw : scrollWidgets) {
            if (sw.widget.isMouseOver(mouseX, mouseY)) {
                if (sw.widget.mouseReleased(event)) {
                    return true;
                }
            }
        }

        // Forward to pinned buttons (outside clip area)
        if (doneButton.mouseReleased(event)) return true;
        if (resetAllButton.mouseReleased(event)) return true;
    ...
    public boolean mouseDragged(MouseButtonEvent event, double dragX, double dragY) {
        ...
        // Forward to the focused SliderWidget (same pattern as old working version)
        for (var sw : scrollWidgets) {
            if (sw.widget.isFocused() && sw.widget instanceof SliderWidget) {
                if (sw.widget.mouseDragged(event, dragX, dragY)) {
                    return true;
                }
            }
        }
