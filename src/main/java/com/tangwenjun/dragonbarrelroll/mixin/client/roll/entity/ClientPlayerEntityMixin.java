...
public abstract class ClientPlayerEntityMixin extends PlayerEntityMixin {
    @Unique
    private float dragon_barrel_roll$renderYaw;
    @Unique
    private float dragon_barrel_roll$lastRenderYaw;
    @Unique
    private boolean dragon_barrel_roll$renderYawInit;

    @Override
    @Unique
    protected void dragon_barrel_roll$baseTickTail2() {
        // Update rolling status
        dragon_barrel_roll$setRolling(RollEvents.shouldRoll());
    }
    ...
    public void dragon_barrel_roll$changeElytraLook(double pitch, double yaw, double roll, Sensitivity sensitivity, double mouseDelta) {
        ...
        var currentRoll = dragon_barrel_roll$getRoll();
        ...
        dragon_barrel_roll$changeElytraLook((float) rotDelta.pitch(), (float) rotDelta.yaw(), (float) rotDelta.roll());
    }
    ...
    public void dragon_barrel_roll$changeElytraLook(float pitch, float yaw, float roll) {
        ...
        if (!dragon_barrel_roll$renderYawInit) {
            dragon_barrel_roll$renderYaw = getYRot();
            dragon_barrel_roll$lastRenderYaw = getYRot();
            dragon_barrel_roll$renderYawInit = true;
        }
        ...
        float oldRenderYaw = dragon_barrel_roll$renderYaw;

        // Apply vanilla pitch and yaw
        turn(deltaX / 0.15, deltaY / 0.15);

        // Update continuous render yaw tracking
        dragon_barrel_roll$lastRenderYaw = oldRenderYaw;
        dragon_barrel_roll$renderYaw = oldRenderYaw + (float) deltaX;

        // Apply roll
        this.dragon_barrel_roll$roll += (float) deltaRoll;
        this.dragon_barrel_roll$prevRoll += (float) deltaRoll;

        // fix hand spasm when wrapping yaw value
        if (getYRot() < -90 && dragon_barrel_roll$renderYaw > 90) {
            dragon_barrel_roll$renderYaw -= 360;
            dragon_barrel_roll$lastRenderYaw -= 360;
        } else if (getYRot() > 90 && dragon_barrel_roll$renderYaw < -90) {
            dragon_barrel_roll$renderYaw += 360;
            dragon_barrel_roll$lastRenderYaw += 360;
        }
    }
